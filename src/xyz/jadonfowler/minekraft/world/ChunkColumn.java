...
public class ChunkColumn {
    ...
    public ChunkColumn(int x, int z, Chunk[] y) {
        ...
        this.z = z;
        for (Chunk c : y)
            if (x == 55 && z == 39) if (c == null || c.getBlocks() == null) c = new Chunk(true);
        this.y = y;
        chunks.add(this);
    }
    ...
    public static void setBlock(Position p, int id) {
        Vector3d v = new Vector3d(p.getX() > 0 ? Math.floor(p.getX() / 16) : -1 + Math.ceil(p.getX() / 16),
                p.getY() > 0 ? Math.floor(p.getY() / 16) : -1 + Math.ceil(p.getY() / 16), p.getZ() > 0 ? Math.floor(p
                        .getZ() / 16) : -1 + Math.ceil(p.getZ() / 16));
        Chunk c = getChunk((int) v.x, (int) v.y, (int) v.z);
        Vector3d b = new Vector3d(Math.floor(p.getX() > 0 ? p.getX() % 16 : 16 - (Math.abs(p.getX()) % 16)),
                Math.floor(p.getY() > 0 ? p.getY() % 16 : 16 - (Math.abs(p.getY()) % 16)), Math.floor(p.getZ() > 0 ? p
                        .getZ() % 16 : 16 - (Math.abs(p.getZ()) % 16)));
        // System.out.println(v + " : " + b + " : " + id);
